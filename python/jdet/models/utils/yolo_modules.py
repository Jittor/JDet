...
class Conv(nn.Module):
    ...
    def print_weights(self):
        ...
        bn_w = self.bn.weight
        bn_b = [v.item() for v in self.bn.bias]
        out_conv = [weight[i].mean().item() for i in range(len(weight))]
        out_bnw = [bn_w[i].mean().item() for i in range(len(bn_w))]
        print("conv's weight's are {}".format(' '.join(map(str,out_conv))))
        print()
        print("bn's weight's are {}".format(' '.join(map(str, out_bnw))))
        print()
        print("nv's bais' are {}".format(' '.join(map(str, bn_b))))
        print()
...
class BottleneckCSP(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = nn.Conv(c1, c_, 1, 1, bias=False)
        self.cv3 = nn.Conv(c_, c_, 1, 1, bias=False)
        self.cv4 = Conv(2 * c_, c2, 1, 1)
        self.bn = nn.BatchNorm(2 * c_)  # applied to cat(cv2, cv3)
...
class C3(nn.Module):
    ...
    def execute(self, x):
        return self.cv3(jt.contrib.concat((self.m(self.cv1(x)), self.cv2(x)), dim=1))
...
class SPP(nn.Module):
    ...
    def execute(self, x):
        x = self.cv1(x)
        return self.cv2(jt.contrib.concat([x] + [m(x) for m in self.m], 1))
...
class Focus(nn.Module):
    ...
    def execute(self, x):  # x(b,c,w,h) -> y(b,4c,w/2,h/2)
        return self.conv(jt.contrib.concat([x[..., ::2, ::2], x[..., 1::2, ::2], x[..., ::2, 1::2], x[..., 1::2, 1::2]], 1))
        # return self.conv(self.contract(x))

    def print_weights(self):
        self.conv.print_weights()    
...
class Detections:
    ...
    def __init__(self, imgs, pred, files,names=None):
        ...
        gn = [jt.array([*[im.shape[i] for i in [1, 0, 1, 0]], 1., 1.]) for im in imgs]  # normalizations
...
class Flatten(nn.Module):
    ...
    def execute(self,x):
        return jt.flatten(x,self.start_dim,self.end_dim)
...
class Classify(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1):  # ch_in, ch_out, kernel, stride, padding, groups
        ...
        self.conv = nn.Conv(c1, c2, k, s, autopad(k, p), groups=g)  # to x(b,c2,1,1)
        self.flat = Flatten()

    def execute(self, x):
        z = jt.contrib.concat([self.aap(y) for y in (x if isinstance(x, list) else [x])], 1)  # cat if list
...
class GhostConv(nn.Module):
    ...
    def execute(self, x):
        y = self.cv1(x)
        return jt.contrib.concat([y, self.cv2(y)], 1)
...
class MixConv2d(nn.Module):
    ...
    def __init__(self, c1, c2, k=(1, 3), s=1, equal_ch=True):
        ...
        if equal_ch:  # equal c_ per group
            i = jt.array(np.linspace(0, groups - 1E-6, c2).as_type(np.float32)).floor()  # c2 indices
        ...
        self.m = nn.ModuleList([nn.Conv(c1, int(c_[g]), k[g], s, k[g] // 2, bias=False) for g in range(groups)])
        self.bn = nn.BatchNorm(c2)
        self.act = nn.LeakyReLU(0.1)

    def execute(self, x):
        return x + self.act(self.bn(jt.contrib.concat([m(x) for m in self.m], 1)))
