...
class R3Head(nn.Module):
    ...
    def _init_layers(self):
        ...
        self.align_conv = FeatureRefineModule(
            self.feat_channels, featmap_strides=[8, 16, 32, 64, 128])
    ...
    def forward_single(self, x, stride):
        init_reg_feat = x
        for init_reg_conv in self.init_reg_convs:
            init_reg_feat = init_reg_conv(init_reg_feat)
        init_bbox_pred = self.init_reg(init_reg_feat)

        # only forward during training
        if self.is_training():
            init_cls_feat = x
            for init_cls_conv in self.init_cls_convs:
                init_cls_feat = init_cls_conv(init_cls_feat)
            init_cls_score = self.init_cls(init_cls_feat)
        else:
            init_cls_score = None

        num_level = self.anchor_strides.index(stride)
        featmap_size = tuple(init_bbox_pred.shape[-2:])
        ...
        init_cls_score, _ = init_cls_score.max(dim=-1, keepdim=True)
        best_ind = init_cls_score.argmax(dim=-2, keepdim=True)
        best_ind = best_ind.expand(-1, -1, -1, 5)

        bbox_pred = bbox_pred.permute(0, 2, 3, 1)
        bbox_pred = bbox_pred.reshape(-1, self.num_anchors, 5)
        best_pred = bbox_pred.gather(
            dim=-2, index=best_ind).squeeze(dim=-2)

        anchors = anchors.reshape(-1, self.num_anchors, 5)

        best_anchor = anchors.gather(
            dim=-2, index=best_ind).squeeze(dim=-2)
        best_bbox = self.bbox_coder.decode(best_anchor,
                                                best_pred)

        # refine_anchor = bbox_decode(
        #     init_bbox_pred.detach(),
        #     init_anchors,
        #     self.target_means,
        #     self.target_stds)

        align_feat = self.align_conv(x, best_bbox.clone())

        or_feat = self.or_conv(align_feat)
        refine_reg_feat = or_feat
        refine_cls_feat = or_feat

        for refine_reg_conv in self.refine_reg_convs:
            refine_reg_feat = refine_reg_conv(refine_reg_feat)
        for refine_cls_conv in self.refine_cls_convs:
            refine_cls_feat = refine_cls_conv(refine_cls_feat)
        refine_cls_score = self.refine_cls(refine_cls_feat)
        refine_bbox_pred = self.refine_reg(refine_reg_feat)

        return init_cls_score, init_bbox_pred, refine_anchor, refine_cls_score, refine_bbox_pred
