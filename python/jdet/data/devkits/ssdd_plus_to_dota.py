...
def xml2txt(xml_path, txt_path, rescale):
...
def ssdd_plus_to_dota(img_path, anno_path, target_path, resize):
    names = []
    ...
    for i in tqdm(range(len(names))):
        ...
        xml2txt(os.path.join(anno_path, name+'.xml'), os.path.join(out_anno_path, name+'.txt'), (resize / w, resize / h))