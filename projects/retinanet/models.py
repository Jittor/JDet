...
@MODELS.register_module()
def YangXuePrameterGroupsGenerator(named_params, conv_bias_grad_muyilpy=1., conv_bias_weight_decay=-1, freeze_prefix=[]):
    normal_group = {
        "params":[],
        "grad_mutilpy":1
    }
    ...
    for p in named_params:
        ...
        if ((("conv" in names[-2]) or ("conv" in names[-3]) or (names[-2] in ['retina_reg','retina_cls'])) and (names[-1] == "bias")):
